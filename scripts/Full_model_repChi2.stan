...
data {
  ...
  int         nu_B;      // shrinkage for Bs
  real         alpha_B;
  real         beta_B;
  int         nu;        // shrinkage for Lambdas
  real         alpha1;
  real         beta1;
  real         alpha2;
  real         beta2;
...
parameters{
  ...
  row_vector[p]         B_psi_std[b,nu_B];          // precision of fixed effect coefficients for Y
  ...
  row_vector[p]          psi_std[K,nu];        // part of the precision mixture contributing to t-distribution on Lambda components
...
transformed parameters{
  ...
  row_vector[p]          psi[K];        // part of the precision mixture contributing to t-distribution on Lambda components
  matrix[b,p]  B_psi;          // precision of fixed effect coefficients for Y

  for(k in 1:K){
    matrix[nu,p]          psi_std2;        // part of the precision mixture contributing to t-distribution on Lambda components
    for(i in 1:nu){
      psi_std2[i] <- columns_dot_self(psi_std[k][i]);
    }
    psi[k] <- rep_row_vector(1.0,nu) * psi_std2;
  }

  for(j in 1:b){
    matrix[nu_B,p]          B_psi_std2;        // part of the precision mixture contributing to t-distribution on Lambda components
    for(i in 1:nu_B){
      B_psi_std2[i] <- columns_dot_self(B_psi_std[j][i]);
    }
    B_psi[j] <- rep_row_vector(1.0,nu_B) * B_psi_std2;
  }
